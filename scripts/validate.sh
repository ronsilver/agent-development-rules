...
validate_frontmatter() {
    ...
    if [[ ! -d "$rules_path" ]]; then
        log_warn "Directorio de rules no encontrado: $rules_path"
        return 0
    fi
    ...
    while IFS= read -r file; do
        if ! has_frontmatter "$file"; then
            log_warn "  Sin frontmatter: ${file#${CONTENT_DIR}/}"
            ((missing_frontmatter++))
        fi
    done < <(find "$rules_path" -name "*.md" -type f 2>/dev/null)
    
    if [[ $missing_frontmatter -gt 0 ]]; then
        log_warn "Se encontraron $missing_frontmatter archivos sin frontmatter"
        WARNINGS=$((WARNINGS + missing_frontmatter))
    else
        log_success "Todos los archivos tienen frontmatter"
    fi
